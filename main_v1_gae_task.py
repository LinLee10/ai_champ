...
def _compute_gae_ref(rewards, values, terminated, truncated, gamma, lam):
    ...
    for t in range(T - 1, -1, -1):
        ...
        for b in range(B):
            ...
            delta = rewards[t][b] + gamma * values[t + 1][b] * boot - values[t][b]
...
- For each t,b:
  ...
  delta[t,b] = rewards[t][b] + gamma * values[t+1][b] * boot - values[t][b]
- Let done[t,b] = terminated[t][b] or truncated[t][b]
  advantages recursion only continues when done is false:
  adv[t,b] = delta[t,b] + gamma * lam * adv[t+1,b] if done[t,b] is false
...
Worked example (gamma=0.9, lam=0.95):
rewards=[[1],[1],[1]]
values=[[0],[0.5],[0],[0]]
terminated=[[False],[False],[True]]
truncated=[[False],[True],[False]]
expected advantages=[[1.8775],[0.5],[1.0]]
